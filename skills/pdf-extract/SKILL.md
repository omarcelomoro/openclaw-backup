---
name: pdf-extract
description: "Extract text from PDF files for LLM processing"
source: LeoYeAI/openclaw-master-skills
status: imported-for-leevre-review
metadata:
 {
 "openclaw":
 {
 "emoji": "📄",
 "requires": { "bins": ["pdftotext"] },
 "install":
 [
 {
 "id": "dnf",
 "kind": "dnf",
 "package": "poppler-utils",
 "bins": ["pdftotext"],
 "label": "Install via dnf"
 }
 ]
 }
 }
---

# PDF Extract

Extract text from PDF files for LLM processing. Uses pdftotext from the poppler-utils package to convert PDF documents into plain text.

## Commands

# Extract all text from a PDF
pdf-extract "document.pdf"

# Extract text from specific pages
pdf-extract "document.pdf" --pages 1-5

## Install

sudo dnf install poppler-utils

## Leevre adaptation notes
- Use as the fast path for extracting raw text from commission statements, insurer PDFs, scripts, and operational documents before structuring in Excel or CRM.
- Validate extracted totals and identifiers before using them in reconciliation.
